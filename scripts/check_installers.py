...
def main() -> int:
    ...
    checks = [
        ...
        ('#if IsChooser == "0"' in iss and "InfoBeforeFile={#InfoBefore}" in iss, "InfoBefore only on dedicated packages"),
        ...
        ("BOTH  —  Server and Client" in iss or "BOTH  —  Server + Client" in iss or "BOTH  —  full Coalesce" in iss, "both option label"),
        ("SERVER ONLY  —  data and API" in iss or "SERVER  —  data and API" in iss or "SERVER ONLY  —  holds the data" in iss or "SERVER ONLY  —  data + API" in iss, "server option label"),
        ("CLIENT ONLY  —  desktop UI" in iss or "CLIENT  —  the screen you work in" in iss or "CLIENT ONLY  —  the desk you work at" in iss, "client option label"),
        ("RoleBadge" in iss and "Recommended" in iss, "recommended badge on Both"),
        ("RolePickBoth" in iss and "SELECTED" in iss, "SELECTED marker on active card"),
        ("RoleSummary" in iss and "You selected:" in iss and "Next installs that" in iss, "live choice summary"),
        ("ChoiceOutcome" in iss and "After install this PC will" in iss, "plain-English outcome line under selection"),
        ("Not sure? Leave Both selected" in iss, "unsure users steered to Both"),
        ("RoleSummary.Font.Color" in iss and "clMaroon" in iss and "clTeal" in iss, "summary bar color tracks selected role"),
        ("RoleSummaryPanel.Color" in iss and "$00E8E8FF" in iss and "$00F0FFF0" in iss, "summary bar background tints by role"),
        ("UpdateNextButtonForRole" in iss and "Install Both" in iss, "Next button spells out role"),
        ("Yes — Install" in iss, "Next button confirms with Yes — Install"),
        ("Pick if:" in iss, "each card has a Pick if audience line"),
        ("colored bar" in iss or "color-tinted" in iss or "Tint the bar" in iss, "chooser copy mentions tinted summary bar"),
        ("CurPageChanged" in iss, "Next button restored off chooser page"),
        ("Host PC" in iss and "Workstation" in iss, "Server/Client card badges"),
        ("WizardKeyDown" in iss and "Key = 49" in iss and "Key = 97" in iss, "1/2/3 keyboard shortcuts"),
        ("CycleRole" in iss and "Key = 38" in iss and "Key = 40" in iss, "Up/Down arrow card cycling"),
        ("Key = 37" in iss and "Key = 39" in iss, "Left/Right arrow card cycling"),
        ("RoleSummaryPanel" in iss, "selection summary sits on a panel"),
        ("Key = 13" in iss and "Key = 32" in iss, "Enter/Space advance from chooser"),
        ("CHOOSE ONE" in iss, "chooser intro says CHOOSE ONE"),
        ("Quick guide:" in iss and "one PC" in iss and "shared host" in iss and "extra desk" in iss, "chooser quick-guide maps situations to cards"),
        ("Includes:" in iss and "Skips:" in iss, "each choice states Includes and Skips"),
        ("BevelWidth := 2" in iss, "selected card uses thicker bevel"),
        ...
        ("RoleNumBoth" in iss and "RoleNumServer" in iss and "RoleNumClient" in iss, "large 1/2/3 number glyphs"),
        ("Click to choose" in iss, "unselected cards say Click to choose"),
        ("This PC will get:" in iss, "ready memo states what this PC gets"),
        ...
        ('Package == "server"' in iss and "Installed: SERVER ONLY" in iss, "server finished copy"),
        ('Package == "client"' in iss and "Installed: CLIENT ONLY" in iss, "client finished copy"),
    ]
