...
def main() -> int:
    ...
    checks = [
        ...
        ('OutputName "CoalesceSetup"' in iss or 'OutputName "CoalesceSetup"' in iss.replace("=", " ="), "combined output name"),
        ...
        ("BOTH  —  Server and Client" in iss, "both option label"),
        ("SERVER  —  data and API" in iss, "server option label"),
        ("CLIENT  —  the screen you work in" in iss, "client option label"),
    ...
    # Dedicated packages must not ship the other role's payload unconditionally.
    # Combined ships both under Components; client/server use #if Has* guards.
