...
|---------|------------------|----------|
| **Combined (chooser)** | Opens on three cards — pick **Both**, **Server**, or **Client** | [CoalesceSetup.exe](https://github.com/devildog5x5/ERP/releases/download/v1.6.39/CoalesceSetup.exe) |
| **Client only** | Desktop UI only (talks to a running Coalesce Server) | [CoalesceClientSetup.exe](https://github.com/devildog5x5/ERP/releases/download/v1.6.39/CoalesceClientSetup.exe) |
| **Server only** | API / database host only | [CoalesceServerSetup.exe](https://github.com/devildog5x5/ERP/releases/download/v1.6.39/CoalesceServerSetup.exe) |

- Latest release: [v1.6.39](https://github.com/devildog5x5/ERP/releases/tag/v1.6.39)
- **CoalesceSetup.exe** opens on **What should this PC run?** — three cards (**1 Both** / **2 Server only** / **3 Client only**; badges: Recommended / Host PC / Workstation). Each card has a **Pick if …** line. Active card shows ✓ SELECTED; the others say “click to pick”; the summary bar restates the choice and says Next installs that; Next becomes **Yes — Install Both →** / **Yes — Install Server →** / **Yes — Install Client →**. Click, press 1–3, arrow keys, Enter / Space, or double-click. Silent: `/TYPE=full|server|client`.
...
Produces three Setup packages under `installers\`:

| File | Role |
|------|------|
| `CoalesceSetup.exe` | Combined chooser — Both / Server / Client |
| `CoalesceClientSetup.exe` | Client only |
| `CoalesceServerSetup.exe` | Server only |

`python scripts/check_installers.py` validates the Inno sources (and package sizes when the EXEs are present).
