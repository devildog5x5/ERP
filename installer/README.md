...
Version lives in `version.iss`. Dedicated packages open with a short InfoBefore page and never show the role chooser. The combined package shows InfoBefore, then three clickable option cards (stock Components page is skipped). The selected card sinks; hints and the Recommended label select that option.
