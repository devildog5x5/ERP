...
Version lives in `version.iss`. Dedicated packages open with a short InfoBefore page and never show the role chooser. The combined package shows InfoBefore, then three numbered option cards (stock Components page is skipped). The selected card sinks and highlights; a live "You selected:" line restates the choice. Click a card, press 1 / 2 / 3, or double-click to select and continue.
