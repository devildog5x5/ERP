...
**Combined** skips Welcome and InfoBefore so the three numbered option cards are the first screen ("What should this PC run?"). A quick guide maps situations to cards (**one PC → Both · shared host → Server · extra desk → Client**); unsure users are told to leave **Both** selected. Large **1 / 2 / 3** glyphs sit on each card; titles are situation-first (**only PC** / **holds the data** / **desk you work at**). Each card states **Includes** / **Skips** plus a **Pick if …** audience line. Selected card sinks, thickens, highlights, and shows ✓ SELECTED; unselected cards say “Click to choose”; a summary bar under the cards is **color-tinted** (yellow / rose / mint) to match the card, restates Includes/Skips plus a plain-English “After install this PC will …” outcome, and says Next installs that; Next becomes **Yes — Install Both →** / **Yes — Install Server →** / **Yes — Install Client →**. Click, press 1 / 2 / 3, use arrow keys, Enter / Space, or double-click to continue. Silent: `/TYPE=full|server|client`. Dedicated Client/Server packages ship smaller payloads with no chooser.
