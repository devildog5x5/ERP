...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'Three obvious choices — pick one card, then Install…');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'CHOOSE ONE card. The Next button becomes Install Both → / Install Server → / Install Client →.'#13#10 +
    '1 = Both (this PC does everything)  ·  2 = Server (host the data)  ·  3 = Client (desk UI only)';
