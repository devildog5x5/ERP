...
begin
  RolePage := CreateCustomPage(wpInfoBefore,
    ...
    'Pick one box — Both, Server, or Client — for this computer.');
  ...
  RoleIntro.Caption :=
    ...
    'you work in every day. Click the option that belongs on THIS PC.';
  ...
  TopY := RoleIntro.Top + RoleIntro.Height + ScaleY(8);
  PanelH := ScaleY(58);
  ...
  RoleHintBoth.Caption :=
    'Full install on this PC. Best for a single-computer shop. Start Server first, then Client.';
  ...
  RoleHintServer.Caption :=
    'Install once where inventory and orders live. Listens on http://127.0.0.1:8000. Login: admin / admin.';
  ...
  RoleHintServer.Height := ScaleY(22);
  ...
  RoleHintClient.Caption :=
    'Install on each workstation. Needs a running Server (this PC or another on your network).';
  ...
  RoleHintClient.Height := ScaleY(22);
  ...
  RoleFoot.Caption :=
    'Not sure? Choose BOTH. You can always add another Client later with CoalesceClientSetup.';
