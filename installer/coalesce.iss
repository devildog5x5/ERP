...
begin
  RolePage := CreateCustomPage(wpInfoBefore,
    ...
    'One choice. The Next button will say Install Both, Install Server, or Install Client.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'SERVER keeps the data and API.  CLIENT is the desktop you work in.  ' +
    'Click a card — or press 1, 2, or 3:';
  ...
  RoleBoth.Caption := '1   BOTH  —  Server and Client on this PC';
  ...
  RoleHintBoth.Caption :=
    'Best for a single PC. Installs the API host and the desktop together.';
  ...
  RoleServer.Caption := '2   SERVER ONLY  —  data and API host';
  ...
  RoleHintServer.Caption :=
    'This PC hosts inventory and orders (http://127.0.0.1:8000). First login: admin / admin.';
  ...
  RoleHintServer.Height := ScaleY(24);
  ...
  RoleClient.Caption := '3   CLIENT ONLY  —  desktop UI only';
  ...
  RoleHintClient.Caption :=
    'For each desk. A Coalesce Server must already be running (here or on the host PC).';
  ...
  RoleHintClient.Height := ScaleY(24);
  ...
  RoleFoot.Caption :=
    'Not sure? Pick BOTH (1). Adding another desk later? Run CoalesceClientSetup.exe there.';
