...
begin
  ...
  RoleIntro.Caption :=
    'Server holds the data. Client is the desktop you work in. ' +
    'Pick what belongs on THIS computer:';
  ...
  RoleHintBoth.Caption :=
    'Everything on one PC. Typical for a single-computer shop. Start Server, then Client.';
  ...
  RoleHintServer.Caption :=
    'Install where inventory and orders live. Default API http://127.0.0.1:8000 · login admin / admin.';
  ...
  RoleHintClient.Caption :=
    'Install on each workstation. Needs a running Server (here or elsewhere on the network).';
