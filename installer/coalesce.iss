...
begin
  RolePage := CreateCustomPage(wpInfoBefore,
    ...
    'Pick one card. Next becomes Install Both / Install Server / Install Client.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'SERVER = data + API host.  CLIENT = the desktop you work in.  ' +
  ...
  RoleFoot.Caption :=
    'Unsure? Choose BOTH (1). Extra desks later: run CoalesceClientSetup.exe on those PCs.';
