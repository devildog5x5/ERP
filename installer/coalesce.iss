...
begin
  ...
  begin
    ...
    if RolePickBoth <> nil then
    begin
      RolePickBoth.Caption := 'click to pick';
  ...
  begin
    ...
    if RolePickServer <> nil then
    begin
      RolePickServer.Caption := 'click to pick';
  ...
  begin
    ...
    if RolePickClient <> nil then
    begin
      RolePickClient.Caption := 'click to pick';
...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'Three obvious choices — Server, Client, or Both. Pick one card, then Install.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'CHOOSE ONE card below. Unselected cards say “click to pick”; the active one says ✓ SELECTED.'#13#10 +
    'Read the “Pick if …” line on each card — then Next becomes Yes — Install … →   ·   keys 1 / 2 / 3';
  ...
  RoleBoth.Caption := '1   BOTH  —  Server + Client on this PC';
  ...
  RoleHintBoth.Width := RoleBothPanel.Width - ScaleX(120);
  ...
  RoleHintServer.Width := RoleServerPanel.Width - ScaleX(120);
  ...
  RoleHintClient.Width := RoleClientPanel.Width - ScaleX(120);
  ...
  RoleFoot.Caption :=
    ...
    'Already sure? Use CoalesceServerSetup.exe or CoalesceClientSetup.exe instead — no chooser, smaller download.';
