...
begin
  RolePage := CreateCustomPage(wpInfoBefore,
    'What will THIS computer run?',
    'Pick one card. Next becomes Install Both, Install Server, or Install Client.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'SERVER = shared data + API.  CLIENT = the desktop you work in.  ' +
    'One PC, one choice — click a card or press 1 / 2 / 3:';
  ...
  RoleHintBoth.Caption :=
    'Pick when this is your only Coalesce PC — API host and desktop UI together.';
  ...
  RoleHintServer.Caption :=
    'Pick when other desks connect here. Shared DB + API stay on this PC (admin / admin).';
  ...
  RoleHintClient.Caption :=
    'Pick when a Server already runs elsewhere — this machine is just a desk.';
  ...
  RoleFoot.Caption :=
    'Unsure? Press 1 (Both). Smaller single-role downloads: CoalesceServerSetup.exe / CoalesceClientSetup.exe.';
