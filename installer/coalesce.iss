...
begin
  ...
  { Loud, plain English — the bar under the cards is the decision receipt.
    Tint the bar itself (not only the text) so the choice is obvious at a glance. }
...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'Pick exactly one card. Only that role installs — nothing else.');
  ...
  RoleIntro.Caption :=
    ...
    'CHOOSE ONE. Each card lists Includes / Skips. ✓ SELECTED + the colored bar = what Next installs.';
  ...
  RoleHintBoth.Caption :=
    ...
    'Pick if: one machine does the work — most people start here.';
  ...
  RoleServer.Caption := 'SERVER ONLY  —  data + API host (no Client)';
  ...
  RoleHintServer.Caption :=
    ...
    'Pick if: this PC is the shared host; desks install Client elsewhere.';
  ...
  RoleHintServer.Height := ScaleY(44);
  ...
  RoleClient.Caption := 'CLIENT ONLY  —  desktop UI (no Server here)';
  ...
  RoleHintClient.Caption :=
    ...
    'Pick if: Server already runs on another PC — this one is just a desk.';
  ...
  RoleHintClient.Height := ScaleY(44);
  ...
  RoleSummaryPanel.Height := ScaleY(36);
  ...
  RoleSummary.Caption :=
    'You selected:  BOTH — Includes: Server + Client  ·  Skips: nothing  —  Next installs that.';
  ...
  RoleSummary.Height := ScaleY(28);

  RoleFoot := TNewStaticText.Create(RolePage);
  RoleFoot.Parent := RolePage.Surface;
  RoleFoot.Caption :=
    'Tip: click a card · press 1 / 2 / 3 · arrow keys · Enter / Space · or double-click to go.'#13#10 +
    'Sure about the role already? Use CoalesceServerSetup.exe or CoalesceClientSetup.exe (smaller, no chooser).';
