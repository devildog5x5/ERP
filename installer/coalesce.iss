...
begin
  RolePage := CreateCustomPage(wpInfoBefore,
    ...
    'What should THIS PC run? Next will say Install Both, Install Server, or Install Client.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'SERVER = data + API.  CLIENT = the desktop you work in.  ' +
  ...
  RoleHintBoth.Caption :=
    'One computer does everything — API host and desktop UI together.';
  ...
  RoleHintServer.Caption :=
    'Put the shared database here. Other PCs install Client only. Login: admin / admin.';
  ...
  RoleHintClient.Caption :=
    'Extra desks only. Needs a Coalesce Server already running on the host PC.';
  ...
  RoleFoot.Caption :=
    'Not sure? Pick BOTH (1). Prefer a smaller download? Use CoalesceServerSetup.exe or CoalesceClientSetup.exe instead.';
