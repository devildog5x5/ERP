...
begin
  ...
  { 49/50/51 = main row; 97/98/99 = numpad; 38/40 = Up/Down; 13 = Enter }
...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'One card. Next spells it out: Install Both → / Install Server → / Install Client →.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'Server keeps the database and API. Client is the desktop you work in.'#13#10 +
  ...
  RoleHintBoth.Caption :=
    'Choose this when one PC does everything — API host and desktop UI together.';
  ...
  RoleHintServer.Height := ScaleY(32);
  ...
  RoleHintClient.Height := ScaleY(32);
  ...
  RoleFoot.Caption :=
    'Not sure? Leave Both selected (1). Prefer a smaller download? Grab CoalesceServerSetup.exe or CoalesceClientSetup.exe instead.';
