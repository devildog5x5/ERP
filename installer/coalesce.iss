...
begin
  if RoleServer.Checked then
    Result := 'SERVER ONLY — API + database on this PC (no desktop UI)'
  else if RoleClient.Checked then
    Result := 'CLIENT ONLY — desktop UI on this PC (Server elsewhere)'
  else
    Result := 'BOTH — Server + Client together on this PC';
end;
...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'CHOOSE ONE: Both, Server only, or Client only — then install.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'Quick guide:  one PC → Both (1)   ·   shared host → Server (2)   ·   extra desk → Client (3)'#13#10 +
    'CHOOSE ONE card below. Click it, press 1–3, or use the arrows. ✓ SELECTED + Next = Yes — Install…';
  ...
  RoleBoth.Caption := 'BOTH  —  Server + Client on this PC';
  ...
  RoleHintBoth.Caption :=
    'Pick if: this is your only Coalesce PC. Gets Server (API/DB) and Client (desktop) together.';
  ...
  RoleHintServer.Caption :=
    'Pick if: this box holds the books. Other PCs install Client and connect here.';
  ...
  RoleHintClient.Caption :=
    'Pick if: Server already runs elsewhere — this PC is only a sign-in desk.';
  ...
  RoleSummary.Caption :=
    'You selected:  BOTH — Server + Client together on this PC  —  Next installs that.';
