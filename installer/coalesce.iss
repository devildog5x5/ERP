...
begin
  if RoleServer.Checked then
    Result := 'SERVER ONLY — this PC hosts the API and database'
  else if RoleClient.Checked then
    Result := 'CLIENT ONLY — desktop UI (needs a running Server elsewhere)'
  else
    Result := 'BOTH — Server and Client on this PC (usual choice)';
end;
...
begin
  ...
  RoleIntro.Caption :=
    'CHOOSE ONE card below. Next changes to Install Both → / Install Server → / Install Client →.'#13#10 +
    '1 = Both (API + desktop here)  ·  2 = Server (host the data)  ·  3 = Client (desk UI only)';
  ...
  RoleSummary.Caption :=
    'You selected:  BOTH — Server and Client on this PC (usual choice)  —  Next installs that.';
  ...
  RoleFoot.Caption :=
    'Tip: click / 1–3 / arrows, then Next (or Enter / Space / double-click).'#13#10 +
    'Already sure of the role? Use CoalesceServerSetup.exe or CoalesceClientSetup.exe — no chooser, smaller download.';
