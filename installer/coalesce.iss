...
begin
  if RoleServer.Checked then
    Result := 'SERVER — this PC hosts the API and database'
  else if RoleClient.Checked then
    Result := 'CLIENT — desktop UI only (Server runs elsewhere)'
  else
    Result := 'BOTH — Server and Client on this same PC';
end;
...
begin
  if RoleSummary = nil then
    exit;
  { Loud, plain English — the bar under the cards is the decision receipt. }
  RoleSummary.Caption := 'You selected:  ' + ChoiceSummary() + '  —  Next installs that.';
end;
...
begin
  { Selected card sinks, thickens, highlights, and shows ✓ SELECTED — hard to miss. }
...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'Pick ONE card — Both, Server only, or Client only — then install.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'CHOOSE ONE card. Large numbers 1 / 2 / 3 — press that key, click the card, or use the arrows.'#13#10 +
    '✓ SELECTED marks your pick. Next becomes Yes — Install Both / Server / Client →';
  ...
  RoleNumBoth.Left := ScaleX(8);
  RoleNumBoth.Top := ScaleY(18);
  ...
  RoleBoth.Caption := 'BOTH  —  full Coalesce on this PC';
  RoleBoth.Font.Name := 'Segoe UI';
  RoleBoth.Font.Size := 12;
  RoleBoth.Font.Style := [fsBold];
  RoleBoth.Left := ScaleX(36);
  RoleBoth.Top := ScaleY(6);
  RoleBoth.Width := RoleBothPanel.Width - ScaleX(148);
  ...
  RolePickBoth.Left := RoleBothPanel.Width - ScaleX(100);
  RolePickBoth.Top := ScaleY(28);
  ...
  RoleHintBoth.Caption :=
    'Pick if: this is the only PC. Installs Server (API/DB) and Client (desktop) together.';
  RoleHintBoth.Font.Name := 'Segoe UI';
  RoleHintBoth.Font.Size := 9;
  RoleHintBoth.Left := ScaleX(36);
  RoleHintBoth.Top := ScaleY(32);
  RoleHintBoth.Width := RoleBothPanel.Width - ScaleX(140);
  RoleHintBoth.AutoSize := False;
  RoleHintBoth.WordWrap := True;
  RoleHintBoth.Height := ScaleY(38);
  ...
  RoleNumServer.Font.Size := 22;
  RoleNumServer.Font.Style := [fsBold];
  RoleNumServer.Font.Color := clGray;
  RoleNumServer.AutoSize := True;
  RoleNumServer.Left := ScaleX(8);
  RoleNumServer.Top := ScaleY(18);
  ...
  RoleServer.Caption := 'SERVER ONLY  —  holds the data';
  ...
  RoleServer.Width := RoleServerPanel.Width - ScaleX(130);
  ...
  RolePickServer.Left := RoleServerPanel.Width - ScaleX(100);
  RolePickServer.Top := ScaleY(28);
  ...
  RoleHintServer.Caption :=
    'Pick if: this box hosts the books. Other PCs install Client and point at this Server.';
  RoleHintServer.Font.Name := 'Segoe UI';
  RoleHintServer.Font.Size := 9;
  RoleHintServer.Left := ScaleX(36);
  RoleHintServer.Top := ScaleY(32);
  RoleHintServer.Width := RoleServerPanel.Width - ScaleX(140);
  RoleHintServer.AutoSize := False;
  RoleHintServer.WordWrap := True;
  RoleHintServer.Height := ScaleY(38);
  ...
  RoleNumClient.Font.Size := 22;
  RoleNumClient.Font.Style := [fsBold];
  RoleNumClient.Font.Color := clGray;
  RoleNumClient.AutoSize := True;
  RoleNumClient.Left := ScaleX(8);
  RoleNumClient.Top := ScaleY(18);
  ...
  RoleClient.Caption := 'CLIENT ONLY  —  the desk you work at';
  ...
  RoleClient.Width := RoleClientPanel.Width - ScaleX(140);
  ...
  RolePickClient.Left := RoleClientPanel.Width - ScaleX(100);
  RolePickClient.Top := ScaleY(28);
  ...
  RoleHintClient.Caption :=
    'Pick if: Server already runs on another machine — this PC is just a sign-in desk.';
  RoleHintClient.Font.Name := 'Segoe UI';
  RoleHintClient.Font.Size := 9;
  RoleHintClient.Left := ScaleX(36);
  RoleHintClient.Top := ScaleY(32);
  RoleHintClient.Width := RoleClientPanel.Width - ScaleX(140);
  RoleHintClient.AutoSize := False;
  RoleHintClient.WordWrap := True;
  RoleHintClient.Height := ScaleY(38);
  ...
  RoleSummaryPanel.Top := RoleClientPanel.Top + RoleClientPanel.Height + ScaleY(8);
  RoleSummaryPanel.Width := RolePage.SurfaceWidth;
  RoleSummaryPanel.Height := ScaleY(32);
  ...
  RoleSummary.Caption :=
    'You selected:  BOTH — Server and Client on this same PC  —  Next installs that.';
  ...
  RoleFoot.Caption :=
    'Tip: click a card, press 1–3, or use the arrows. Confirm with Yes — Install…, Enter, Space, or double-click.'#13#10 +
    'Already know the role? CoalesceServerSetup.exe / CoalesceClientSetup.exe skip this screen (smaller download).';
...
begin
  ...
  if CurPageID = wpFinished then
    WizardForm.FinishedLabel.Caption :=
      'Coalesce Server is installed.'#13#10#13#10 +
      'Default login: admin / admin'#13#10 +
      'API: http://127.0.0.1:8000'#13#10#13#10 +
  ...
  if CurPageID = wpFinished then
    WizardForm.FinishedLabel.Caption :=
      'Coalesce Client is installed.'#13#10#13#10 +
