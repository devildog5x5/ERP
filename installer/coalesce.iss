...
begin
  if RoleServer.Checked then
    ...
    Result := 'BOTH — Server + Client on this PC';
end;
...
begin
  RolePage := CreateCustomPage(wpInfoBefore,
    'What do you want on this PC?',
    'Pick one. Click a card, press 1 / 2 / 3, or double-click to continue.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'SERVER = where the data and API live.  CLIENT = the desktop you work in.  ' +
    'Choose what belongs on THIS computer:';
  ...
  RoleHintBoth.Caption :=
    'Full Coalesce on one machine. Usual for a single-PC shop — start Server, then open Client.';
  ...
  RoleHintServer.Caption :=
    'Where inventory and orders live. API http://127.0.0.1:8000 · first login admin / admin.';
  ...
  RoleHintClient.Caption :=
    'On each workstation. Needs a Coalesce Server already running (this PC or another).';
