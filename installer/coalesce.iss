...
{ Keys 1 / 2 / 3 pick a card; Up / Down cycle while the chooser is showing. }
procedure WizardKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (RolePage = nil) or (WizardForm.CurPageID <> RolePage.ID) then
    exit;

  { 49/50/51 = main row; 97/98/99 = numpad; 38/40 = Up/Down }
...
begin
  ...
  RoleIntro.Caption :=
    'Server = shared database + API.  Client = the Windows desktop you work in.'#13#10 +
    'Click a card, press 1 / 2 / 3, or use ↑ ↓:';
  ...
  RoleFoot.Caption :=
    'Not sure? Leave Both selected (press 1). Want a smaller download later? Use CoalesceServerSetup.exe or CoalesceClientSetup.exe.';
