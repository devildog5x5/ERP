...
begin
  { Selected card sinks, highlights, and shows SELECTED — hard to miss. }
...
{ Keys 1 / 2 / 3 pick a card; Up / Down cycle; Enter advances from the chooser. }
procedure WizardKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (RolePage = nil) or (WizardForm.CurPageID <> RolePage.ID) then
    exit;

  { 49/50/51 = main row; 97/98/99 = numpad; 37/38/39/40 = arrows; 13 = Enter }
...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'Pick exactly one card. Next becomes Install Both → / Install Server → / Install Client →.');
  ...
  RoleIntro.Caption :=
    ...
    'Click a card, press 1 / 2 / 3, or use the arrow keys. Enter or double-click continues.';
  ...
  RoleSummary.Caption := 'You selected:  BOTH — Server and Client on this PC (usual choice)';
