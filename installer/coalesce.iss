...
begin
  ...
  RolePage := CreateCustomPage(wpWelcome,
    ...
    'Server, Client, or both — one card, then Install.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'CHOOSE ONE card. Unselected cards say “click to pick”; the active one says ✓ SELECTED.'#13#10 +
    'Next becomes Install Both → / Install Server → / Install Client →   ·   keys 1 / 2 / 3';
  ...
  RoleHintBoth.Caption :=
    'Usual single-PC install: API/database and the desktop UI on this machine.';
  ...
  RoleHintClient.Caption :=
    'Server already runs elsewhere — this machine is only a desk that signs in.';
  ...
  RoleFoot.Caption :=
    'Tip: click a card, press 1–3, or use the arrows — then Next / Enter / Space (or double-click).'#13#10 +
    'Know the role already? Grab CoalesceServerSetup.exe or CoalesceClientSetup.exe — no chooser, smaller download.';
