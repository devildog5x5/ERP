...
begin
  RolePage := CreateCustomPage(wpInfoBefore,
    ...
    'One choice for this PC. Click a card, press 1 / 2 / 3, or double-click to continue.');

  RoleIntro := TNewStaticText.Create(RolePage);
  RoleIntro.Parent := RolePage.Surface;
  RoleIntro.Caption :=
    'SERVER keeps the data (API).  CLIENT is the desk you work at.  ' +
    'Pick exactly one option below:';
  ...
  RoleHintBoth.Caption :=
    'One-PC shop: API + desktop together. Start Server, then open Client.';
  ...
  RoleHintServer.Caption :=
    'Holds inventory and orders. Listens on http://127.0.0.1:8000 · login admin / admin.';
  ...
  RoleHintClient.Caption :=
    'Each desk. Needs Coalesce Server already running (here or on another PC).';
  ...
  RoleFoot.Caption :=
    'Unsure? Press 1 for BOTH. More desks later: install CoalesceClientSetup on those PCs.';
