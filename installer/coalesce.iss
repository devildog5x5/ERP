...
begin
  ...
  RoleIntro.Caption :=
    'CHOOSE ONE of the three cards below. Keys 1 / 2 / 3 jump to a card. The active one shows ✓ SELECTED.'#13#10 +
    'Each card has a “Pick if …” line. Next becomes Yes — Install Both / Server / Client →';
  ...
  RoleHintBoth.Caption :=
    'Pick if: one PC does everything. Installs Server (API/DB) + Client (desktop) here.';
  ...
  RoleServer.Caption := 'SERVER ONLY  —  data and API';
  ...
  RoleHintServer.Caption :=
    'Pick if: this machine holds the books. Other desks install Client and connect here.';
  ...
  RoleHintClient.Caption :=
    'Pick if: Server already runs elsewhere — this PC is only a desk that signs in.';
