...
begin
  ...
  RoleSummary.Caption :=
    'You selected:  ' + ChoiceSummary() + #13#10 +
    ChoiceOutcome() + '  —  Next installs that.';
  if RoleServer.Checked then
  begin
...
begin
  ...
  RoleIntro.Caption :=
    ...
    'CHOOSE ONE. Not sure? Leave Both selected and click Next. ✓ SELECTED + the colored bar = what installs.';
  ...
  RoleSummaryPanel.Top := RoleClientPanel.Top + RoleClientPanel.Height + ScaleY(6);
  ...
  RoleSummary.Caption :=
    'You selected:  BOTH — Includes: Server + Client  ·  Skips: nothing'#13#10 +
    'After install this PC will run the FULL app — Server and Client together. Best if unsure.  —  Next installs that.';
